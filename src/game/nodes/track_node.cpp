...
#pragma region Helpers
	...
	std::vector<track::Edge> TrackNode::populateTrack(sf::Vector2f viewSize, uint16_t difficulty) {
		...
		auto createPoleNode = [&](double left, track::PoleInside inside) {
		...
		while (gateTop <= gateBottom) {
			...
			// Determine which side of the pole represents the inside of the gate.
			auto firstInside  = gateDirection > 0.0 ? track::PoleInside::Left : track::PoleInside::Right;
			auto secondInside = gateDirection > 0.0 ? track::PoleInside::Right : track::PoleInside::Left;

			// Create the inner and outer pole.
			createPoleNode(gateLeft, firstInside);
			createPoleNode(outerLeft, secondInside);
