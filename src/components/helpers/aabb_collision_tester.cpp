...
#pragma region Collision handling
	...
	collision::Result AABBCollisionTester::test(const AABBCollisionTester &other) const {
		...
		for (int i1 = 0; i1 < POINTS_COUNT; i1++) {
			...
			auto p0{ globalPoint(i1) };
			auto p1{ globalPoint(i1e) };

			for (int i2 = 0; i2 < POINTS_COUNT; i2++) {
				int i2e = (i2 + 1) % POINTS_COUNT;
				auto p2{ other.globalPoint(i2) };
				auto p3{ other.globalPoint(i2e) };
		...
		auto ourPos	   = m_shape.getPosition();
		auto ourSize   = m_shape.getSize();
		auto otherPos  = other.m_shape.getPosition();
		auto otherSize = other.m_shape.getSize();

		collision::Result result = static_cast<collision::Result>(0);

		if (ourPos.y < otherPos.y + otherSize.y && ourPos.y + ourSize.y > otherPos.y) {
			if (ourPos.x < otherPos.x) {
				result |= collision::Result::OtherLeft;
			} else if (ourPos.x > otherPos.x) {
				result |= collision::Result::OtherRight;
			}
		}

		if (ourPos.x < otherPos.x + otherSize.x && ourPos.x + ourSize.x > otherPos.x) {
			if (ourPos.y < otherPos.y) {
				result |= collision::Result::OtherTop;
			} else if (ourPos.y > otherPos.y) {
...
#pragma region Getters & setters
	...
	//-------------------------------------------------------------------------
	sf::Vector2f AABBCollisionTester::globalPoint(int index) const {
